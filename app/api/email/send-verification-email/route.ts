...
export async function POST(request: NextRequest) {
	try {
        ...
        if (!email) {
            ...
            return NextResponse.json({ error: "Email is required" }, { status: 400 }); // 400 Bad Request
        }
		...
		return NextResponse.json({ error: "Failed to send email" }, { status: 500 }); // 500 Internal Server Error
	}
}
