...
import { SQLEvent, ContactFormInput, SocietyRegisterFormData, UserRegisterFormData, SQLRegistrations, OrganiserAccountEditFormData, InsertTokenResult } from './types';
...
export async function insertToken(email: string, token: string, purpose: string): Promise<InsertTokenResult> {
    try {
